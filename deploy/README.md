...
**Задачи по расписанию.** Готовое расписание — `deploy/crontab.txt` (пути уже
под `~/margoads`): в 10:00 по Варшаве (`CRON_TZ=Europe/Warsaw`, переход на
зимнее время ничего не сдвигает) синхронизация структуры, сразу за ней — сбор
статистики. Обе команды в одной строке через `;`, как на основном аккаунте:
отчёты стартуют, как только закончился синк, и не нужно угадывать, сколько он
займёт. Отправка изменений в Amazon по расписанию не идёт — только вручную
со страницы «Очередь изменений» или командой
`venv/bin/python scripts/send.py --account MERCH`.

...
### static/common.css — общее оформление (август 2026)

Общая часть оформления вынесена в **`static/common.css`** и подключается на каждой
странице первой строкой в `<head>`:

```html
<link rel="stylesheet" href="/static/common.css?v=1">
<style>
  /* здесь остаётся только то, что специфично для этой страницы */
</style>
```

Там лежат: CSS-переменные, reset, header, nav, `main`, кнопки, таблицы,
фильтр-панели, дропдауны портфолио, бейджи, модалки, тосты — всё, что было
одинаковым на страницах.

**⚠️ CSS подставляется в страницу сервером, а не грузится отдельным запросом.**
Домен панели — `ads.nartdsgncorp.com`, и блокировщики рекламы (Adblock и
подобные) режут любой подзапрос на домен, начинающийся с `ads.`: в DevTools
это видно как `common.css (blocked:other)`, 0 байт, и страница остаётся без
оформления. Поэтому в `app.py` есть `after_request`-хук `inline_common_css`:
он заменяет `<link href="/assets/common.css">` на `<style>` с содержимым файла.
Отдельного запроса нет — блокировать нечего, и правка `static/common.css`
по-прежнему расходится на все страницы (файл читается с кешем по mtime).

Роут `/assets/<file>` тоже остался — им можно проверить содержимое файла
напрямую, а `/assets-check` показывает, какой путь видит приложение.

**Правила работы:**
- `common.css` подставляется **до** `<style>` страницы, поэтому локальное правило
  всегда переопределяет общее. Нужно отступить от общего стиля — просто задайте
  свойство в `<style>` страницы.
- Новая страница должна содержать строку `<link rel="stylesheet"
  href="/assets/common.css?v=1">` — хук найдёт её и подставит стили.
- Новую страницу начинать с подключения `common.css`, а в `<style>` писать только
  её собственные блоки.
- Меняете `common.css` — правка расходится по всем страницам, проверяйте
  несколько разных (аналитика, импорт, control).
- Кеш: при заметных правках поднимать версию в `?v=1` во всех страницах, иначе
  браузеры отдадут старый файл.

**Как это собиралось (если понадобится повторить):** свойство уезжало в
`common.css`, только когда оно задано на **всех** страницах, где встречается
селектор, а сам селектор не используется там, где правила для него нет.
Иначе стиль «протёк» бы на страницу, где его раньше не было. Результат
проверялся пиксельным сравнением всех страниц в Chromium до и после.

### CSS-переменные (общие — уже в common.css)
...
- Клик по строке → inline-панель кампаний. Раскрытых строк может быть сразу
  несколько (`S.eKeys` — Set): повторный клик сворачивает только свою строку,
  соседние остаются открытыми. Полный сброс — `closeExpand()` при перезагрузке.
- Клик по кампании → структура групп с вкладками
- Полное управление кампаниями: toggle, inline bid/budget/name, + Группа, + KW, + Минус
- **Режим массовой остановки** (чекбокс «Массовая остановка» в конце фильтров):
  включает чекбоксы у ASIN, групп и таргетов. Чекбокс у ASIN выбирает все его
  активные группы разом — список берётся из `/automation/pause-asins/groups`,
  снятие галочки убирает именно их (`S.asinGrp`). Выбранное (`S.bulk`) ставится на паузу
  пачкой через `/control/add_batch` — та же очередь изменений, что и у
  одиночных переключателей. Уже остановленные объекты выбрать нельзя, смешивать
  маркетплейсы — тоже (у каждого свой `profile_id`). Открытые структуры
  перерисовываются из кэша `S.sc` (`refreshStructs()`), запросы не повторяются,
  раскрытые группы остаются раскрытыми, а чекбоксы у ASIN добавляются на месте
  (`refreshAsinCbs()`) — перерисовка таблицы свернула бы раскрытые строки.
  У строк таргетов в этом режиме появляется отдельная колонка под чекбокс
  (`.ih.bulk`/`.ii.bulk`), иначе сетка съезжает и правые колонки уходят за край.
...
- Таблица укладывается в ширину экрана без горизонтальной прокрутки: длинные
  названия группы, кампании и портфолио обрезаются многоточием
  (`.name-main`/`.name-sub` — 190px, `.pf-cell` — 120px), полное значение
  видно в подсказке `title`. Отступы ячеек сжаты до 7px по горизонтали.
  В режиме таргетов текстовых колонок три (таргет, группа, кампания), поэтому
  таблица получает класс `tgt-mode` и они ужимаются до 155px.
...
python3 scripts/auto_collect.py --batch 1                # строго по одному отчёту
```

**Фазы:** создаёт все отчёты сразу → общий поллинг каждые 30 сек →
по мере готовности скачивает и грузит в BigQuery. Полный прогон ≈ 15 мин
(56 отчётов), большая часть времени — ожидание Amazon.

**`--batch N` — последовательная обработка.** Список (профиль × тип) режется на
пачки по N: заказали → дождались → загрузили → следующая пачка. `--batch 1` =
строго по одному. Без ключа поведение прежнее (всё сразу, быстрее). Нужно там,
где профилей много и Amazon начинает ограничивать частоту. По умолчанию обе
установки работают одинаково — все отчёты сразу.

**Настройка задачи (PythonAnywhere → Tasks):**
- расписание **daily**, время в **UTC**; рекомендуется `10:00 UTC` — к этому
  моменту вчерашний день закрыт во всех гео, включая US (UTC-8)
...
Ручные отчёты пишутся в `reports_log.json` c `source: "manual"`.
Страница `/ads` показывает объединённую историю через `GET /ads/history?limit=200`.

**Статусы:** `LOADED` (загружено) · `EMPTY` (отчёт пустой) · `FAILED` (Amazon
отказал при создании, в поле error — тело ответа) · `ERROR` (сорвалось
скачивание/загрузка после 3 попыток) · `TIMEOUT` (не готов за 2 часа).

**Пустой отчёт не стирает данные:** при 0 строк `DELETE` не выполняется, если
...
(`del` + `gc.collect()`): структура крупного аккаунта занимает гигабайты,
параллельный обход упёрся бы в лимит 3 ГБ.

**Задача (PythonAnywhere → Tasks):** daily, время в UTC, команда тем же
интерпретатором, что и веб-приложение. Ставить **не одновременно** с
`auto_collect.py` — обе задачи тяжёлые по памяти.

**История:** `campaigns_sync_log.json` — общий файл для ручных синхронизаций
(кнопка на `/campaigns`, `source: "manual"`) и авто (`source: "auto"`).
Страница показывает последние 100 записей через `GET /campaigns/history?limit=100`.

**Статусы:** `OK` · `EMPTY` (API вернул 0 объектов) · `ERROR` · `RUNNING`
(идёт прямо сейчас; если остался висеть — процесс оборвался).

**Пустой ответ не стирает структуру:** `DELETE FROM ... WHERE marketplace` не
...
`/campaigns/preview` — превью 50 строк из BQ дублировало страницы аналитики.
...
**Замена ASIN в названиях (июль 2026):**
- `mapAsinsInText(text, srcAsin, dstAsin)` заменяет ASIN исходного гео на ASIN
  целевого во **всех** названиях групп и кампании (не только asin группы), беря
  соответствие из `asinMap` (каталог + ручные правки); неизвестные ASIN и KDP не трогает.
- Токен ASIN: `ASIN_TOKEN_RE = /(?<![A-Z0-9])B[0-9A-Z]{9}(?![A-Z0-9])/g`.
  Разделителем считается любой не-алфанумерик — пробел, тире, `_`, `/`, начало/конец.
  Раньше стояло `\bB[0-9A-Z]{9}\b`, но `\b` считает `_` частью слова, поэтому в
  именах вида `BXXXXXXXXX_VINTAGE_LABEL` ASIN не заменялся (см. решённые проблемы).
- **Пропуск немаппленных групп:** если для MERCH-группы не нашлось соответствия
  (`at !== 'KDP' && srcAsin && !asinMap[srcAsin]`) — группа помечается `_skip`,
  исключается из payload, показывается в превью блоком «⚠ Будут пропущены».
  Если пропущены все группы — сабмит блокируется с подсказкой указать ASIN вручную.
...
- Пагинация (100/200/500/1000), колонки «Показы», «ASIN кл.», «Ставка → реком.»,
  «CPC» (cost/clicks); у названия кампании и группы — индикатор активности (точка).
- Заметки и «Блок» были убраны (были только клиентскими, в рамках сессии).
- В раскрытии минусы группы и подсветка «в минусах» показывают тип минуса
  (exact/phrase/broad/product) — `/negatives` возвращает `texts` как
  `{текст: тип}` + `text_list`.

### Правила: тай-брейк при пересечении
Если под строку подходят несколько правил с одинаковым приоритетом и областью,
выбор между ними неустойчив. Чтобы рекомендации были детерминированными —
держи диапазоны возраста непересекающимися либо задавай разные `priority`
(меньше число = выше приоритет) / используй портфолио-правила.
...
## Как обновлять код на серверах (чтобы ничего не сломать)

Установок две, код у них **общий**, различия — только в `config/`:

| | Основной аккаунт | Margo |
|---|---|---|
| Где | PythonAnywhere, `~/amazon-ads` | Google Cloud VM `margoads`, `~/margoads` |
| Проект BigQuery | `amazon-ads-api-494412` | `amazon-ads-api-494412` (тот же!) |
...
Ничего не правим в коде «под установку». Всё, что отличает одну установку от
другой (проект, датасет, логин, пароль, название), живёт в
`config/settings.json`. Как только правка уезжает в код — следующее обновление
её затрёт, а в худшем случае копия начнёт писать в чужой датасет.

### Что нельзя трогать при обновлении

- **`config/`** — ключи Amazon, ключ BigQuery, `settings.json`. Папки нет в git,
  и все скрипты обновления её исключают.
- **Данные в BigQuery** — обновление кода их не касается: ни `init_bigquery.py`,
  ни скрипты обновления не выполняют DDL и DELETE.
- **`uploads/`, `logs/`, `*_log.json`** — история загрузок конкретной установки.

### Основной аккаунт (PythonAnywhere)

```bash
cd ~/amazon-ads && git pull
python3 scripts/check_install.py      # ожидаем: проект amazon-ads-api-494412, датасет amazon_ads
```
Затем вкладка **Web** → **Reload**. Без Reload uwsgi продолжает держать старый
код в памяти.

### Margo (Google Cloud VM)

```bash
cd ~/code-src && git pull                          # отдельный клон репозитория
~/code-src/deploy/update_margoads.sh ~/code-src    # обновление ~/margoads
```

Скрипт по шагам: проверяет `config/settings.json` → архивирует `config/` →
переносит файлы (мимо `config/`, `uploads/`, `logs/`, `venv/`) → ставит
зависимости → `check_install.py` → перезапуск. Если проверка не прошла, сервис
**не** перезапускается и продолжает работать прежняя версия.

`sudo` на этой машине запрещён, поэтому перезапуск идёт сигналом gunicorn:
`kill -HUP <мастер>`. Вручную то же самое — `pkill -HUP -f 'gunicorn.*app:app'`.

### Проверка после любого обновления

```bash
python3 scripts/check_install.py
```

Показывает проект, датасет, откуда взята настройка, наличие ключей и таблиц.
Первое, на что смотреть, — строка **датасет**: `amazon_ads` на основном,
`margoads` у Margo. Если у копии виден `amazon_ads` — остановите cron и
проверьте `config/settings.json`, иначе данные пойдут в чужую базу.

### Грабли, на которые уже наступали

1. **`sed`-замены в старом `update_margoads.sh`** (`"amazon_ads"` → `"margoads"`,
   логин и пароль в `app.py`) — больше не нужны и не работают: файлы теперь в
   папках `analytics/`, `management/`, а `sed` шёл по `*.py` в корне. Старый
   скрипт использовать нельзя.
2. **Порядок**: сначала `config/settings.json`, потом код. Иначе копия секунду
   поработает с датасетом по умолчанию.
3. **`rsync` без `--exclude config`** затрёт ключи. В скриптах исключения уже
   прописаны; при ручном копировании их легко забыть.
4. **Reload/перезапуск**: код обновлён ≠ работает. PythonAnywhere — Reload,
   Margo — HUP gunicorn.
5. **Имена полей в очереди изменений**: ставка группы — `default_bid`, не `bid`.
   Очередь теперь сама приводит имя и отклоняет чужие поля, но при добавлении
   новых операций сверяйтесь с `ALLOWED_OPS`.
6. **Если Amazon начнёт ограничивать частоту** на установке с многими
   профилями (у Margo 14 профилей = 56 отчётов разом) — в команду сбора
   добавляется `--batch 8`, и отчёты идут пачками. По умолчанию обе установки
   заказывают всё сразу.

### Если всё-таки сломалось

```bash
# журнал сервиса (Margo)
journalctl -u margoads -n 50 --no-pager

# откат кода на предыдущий коммит
cd ~/code-src && git log --oneline -5
git checkout <предыдущий_коммит>
~/code-src/deploy/update_margoads.sh ~/code-src

# восстановить настройки из архива, который делает скрипт обновления
tar xzf ~/margoads-config-<дата>.tar.gz -C ~/margoads
```

Данные BigQuery при этом не страдают ни в одном сценарии — откатывается только
код.
...
- `deploy/update.sh` — обновление уже работающей копии: архив `config/`, откат
  локальных правок в stash, `git pull`, зависимости, проверка и только потом
  перезапуск сервиса.
- `deploy/update_margoads.sh` — обновление копии margoads (`~/margoads`,
  сервис `margoads`): у неё тот же проект, но датасет `margoads`, поэтому
  скрипт требует `config/settings.json` до заливки кода.
- `scripts/check_install.py` — показывает, с каким проектом и датасетом работает
  установка, откуда взята настройка, на месте ли ключи и таблицы. Отдельно
  предупреждает, если `config/settings.json` нет и установка работает с проектом
  по умолчанию — для второй копии это означает запись в чужую базу.
...
потратило и заработало за период и как это изменилось к предыдущему такому же
периоду.

- Переключатели: аккаунт MERCH/KDP, «Все страны» / «Разбить по странам»,
  период 7/14/30/90 дней. По умолчанию — активные кампании за 30 дней.
- KPI сверху: расход, продажи 14d, заказы, ACoS. У расхода рост подсвечивается
  красным, у продаж и заказов — зелёным.
- Таблица: портфолио (+ страна в режиме разбивки), кампаний, клики, расход,
  продажи, заказы, ACoS, прибыль (продажи − расход). Сортировка по клику на
  заголовке, строка «Итого» внизу.
- Под таблицей — период сравнения и когда последний раз отработали загрузка
  статистики (`auto_collect_log.json`) и синхронизация (`campaigns_sync_log.json`).

### Endpoint
| Метод | URL | Описание |
|---|---|---|
| `GET` | `/dashboard/portfolios` | строки по портфолио, итоги, период, статус загрузок |

Параметры: `account_type`, `date_from`, `date_to`, `by_country=1`,
`marketplace`, `active_only=0`.

**Один запрос на оба периода:** статистика берётся сразу за
`[prev_from, date_to]`, а текущий и предыдущий периоды разделяются внутри
агрегатов через `IF(s.date >= date_from, ...)` — второй запрос не нужен.
Источник цифр тот же, что и на «Аналитике кампаний»: дедуплицированные
кампании + `targets_stats_*` + актуальные имена из `portfolio_labels`.

---

## Остановка ASIN без продаж (automation/pause_asins_routes.py)

Страница `/automation/pause-asins` — массовая остановка рекламы ASIN, которые
тратят бюджет без заказов. Раньше это делалось вручную: отфильтровать на
`/analytics/products`, затем открывать каждый ASIN и останавливать группы по одной.

### Как работает
1. Критерии: аккаунт, маркетплейс, **тип кампаний** (все / только авто /
   только ручные), период, «кликов от», «расход от $», «заказов не больше» и
   «показать» (200 по умолчанию, до 5000). По умолчанию — 30 дней, ≥10 кликов,
   0 заказов. Если список упёрся в лимит, счётчик над таблицей об этом пишет.
2. Таблица найденных ASIN с чекбоксами: показы, клики, расход, продажи, заказы,
   **число активных групп** и разбивка авто/ручные. ASIN без активных групп в
   выборку не попадают.
2б. У каждой группы в раскрытии — чекбокс. Снятая галочка исключает группу из
   остановки (`S.skip`): она не уйдёт в очередь, счётчик в панели действия и
   предпросмотр это учитывают. У уже неработающих групп чекбокс заблокирован.
   Чекбокс «Только активные» (включён по умолчанию, как на «Аналитике товаров»)
   убирает из раскрытия остановленные группы и кампании.
2а. Стрелка слева раскрывает строку: кампании ASIN с типом, состоянием и
   статистикой за период, внутри каждой — её группы. Клик по группе открывает
   её содержимое вкладками «Таргеты / Поисковые запросы / Минус-слова» — данные
   берутся тем же `/analytics/campaigns/structure`, что и на «Аналитике товаров»,
   поэтому содержимое совпадает.
3. Кнопка «Остановить группы» → предпросмотр: список групп с кампаниями и
   пометкой тех, где рекламируются **чужие ASIN** (их реклама тоже встанет).
4. Подтверждение ставит паузу групп в очередь `pending_changes` пачкой.
   Применяется после одобрения в «Очереди изменений» — прямо в Amazon ничего
   не уходит.
...
| `POST` | `/automation/pause-asins/groups` | группы выбранных ASIN для предпросмотра |
| `GET` | `/automation/pause-asins/detail` | кампании и группы одного ASIN (раскрытие строки) |

**Фильтр по типу кампаний** (`ttype=AUTO|MANUAL`) действует и на поиск ASIN, и
на то, какие группы уйдут в очередь, и на раскрытие строки.

**Осторожно с именем CTE `groups`:** `GROUPS` — зарезервированное слово
BigQuery, запрос падает с `Syntax error`. В коде CTE называется `grps`.

**Статистика в строке ASIN** при включённом «Только активные» (по умолчанию)
считается только по группам, работающим сейчас (`live_grp`): иначе в строке
виден расход давно остановленных групп и цифры расходятся с «Аналитикой
товаров». `active_only=0` возвращает расход за период целиком.

**Только активные сущности:** в выборку идут `product_ad` с `ad_state=ENABLED`,
`ad_group` с `ad_group_state=ENABLED` и `campaign` с `campaign_state=ENABLED`
(по последнему `synced_at`) — вся цепочка целиком, иначе группа активной не
считается. Поэтому уже остановленное не предлагается останавливать повторно.
Раскрытие строки (`/detail`) наоборот показывает **всё**, что тратило за период,
включая остановленное: это история расходов, состояние помечено бейджем.

**Один маркетплейс за раз:** у каждого свой `profile_id`, поэтому смешанный
выбор страница не пропускает.

### Ставка группы — только `default_bid`
У Amazon ставка группы называется `defaultBid`, и `ALLOWED_OPS["ad_group"]`
знает `default_bid`. Страницы какое-то время слали её как `bid`: очередь такие
записи принимала (поле не валидировалось), а `send.py` не понимал — в логе
отправки шло «Неизвестный тип: ad_group/bid, пропускаем» по строке на каждую
запись, ставка до Amazon не доходила, а записи оставались в очереди навсегда.

Починено в трёх местах:
- `_canon_field()` в `control_routes.py` приводит `ad_group/bid` → `default_bid`
  на входе во все три эндпоинта очереди;
- там же добавлена валидация `field_name` по `ALLOWED_OPS` — чужое поле теперь
  отклоняется с понятным текстом, а не оседает мёртвым грузом;
- `group_changes()` в `send.py` понимает старое имя, поэтому уже накопившиеся
  записи уходят в Amazon без ручной правки таблицы.

Страницы (`targets.html`, `campaigns_analytics.html`, массовое изменение ставок)
шлют правильное имя поля сами.

### Изменение в /control/add_batch
Раньше пакет принимал только `NO_DUP_CHECK` типы, а `ad_group` отвергался —
для массовой паузы это не годилось. Теперь принимаются любые типы из
`ALLOWED_OPS`, а дубли для типов с проверкой отсекаются **одним** запросом на
весь пакет (по одному запросу на элемент было бы слишком долго). В ответе
появилось поле `skipped` — сколько записей пропущено как уже стоящие в очереди.

**Тест:** `python3 tests/test_pause_asins.py` — страница, критерии в SQL,
предпросмотр с чужими ASIN, пакетная постановка, отсутствие дублей при повторе.
...
- **ASIN не заменялся в названиях групп** (копирование кампаний): регулярка
  использовала `\bB[0-9A-Z]{9}\b`, но `\b` считает `_` частью слова — в именах
  вида `BOGJ6XFBZ8_VINTAGE_LABEL` после ASIN стоит `_`, границы нет, токен не
  матчился и не заменялся (маппинг находил целевой ASIN, но в названии оставался
  старый). Чинится заменой `\b` на `(?<![A-Z0-9])...(?![A-Z0-9])` — `_` и любой
  не-алфанумерик считаются разделителем. На основном аккаунте баг не проявлялся
  из-за иного формата имён (ASIN отделён пробелом/дефисом).

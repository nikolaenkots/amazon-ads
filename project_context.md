...
2а. Стрелка слева раскрывает строку: кампании ASIN с типом, состоянием и
   статистикой за период, внутри каждой — её группы (как на «Аналитике товаров»).
...
и `ad_group` с `ad_group_state=ENABLED` (по последнему `synced_at`), поэтому
уже остановленное не предлагается останавливать повторно.
